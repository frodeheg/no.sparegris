...
class Textify extends PNG {
  ...
  sample(src, startx, starty, w, h, x, y) {
    ...
    const isBg = (x < -0.5) || (y < -0.5) || (x > w - 0.5) || (y > h - 0.5)
     || (a === 1 && (!this.textUnder || y !== (charHeight - 1)));
  ...
  async drawLineChart(x, y, w, h, data) {
    ...
    const yspan = h - padY - 2 * charHeight - ('class' in data ? onIndicatorHeight : 0);
    ...
    // Draw enabled bar
    if ('class' in data) {
      await this.addText(data.classText, x + padX - (data.classText.length + 1) * charWidth, y + h - padY - (onIndicatorHeight / 2) - charHeight / 2);
      for (let i = padX + 1; i < w; i++) {
        const hour = Math.floor(((i - padX) / (w - padX)) * data.values.length);
        if (data.class[hour] !== undefined) {
          await this.setTextColor(data.classCol[data.class[hour]]);
