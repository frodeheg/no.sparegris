...
const CHAR_IMG = '../assets/images/UTF-8-9x17.png';
...
class Textify extends PNG {
  ...
  sample(src, startx, starty, w, h, x, y) {
    ...
    const isBg = (x < -0.5) || (y < -0.5) || (x > w - 0.5) || (y > h - 0.5)
     || (r === 0 && g === 0 && b === 0 && (!this.textUnder || y !== (charHeight - 1)));
    return isBg ? this.textBg : {
      r: (this.textCol.r * (r + 1)) >> 8,
      g: (this.textCol.g * (g + 1)) >> 8,
      b: (this.textCol.b * (b + 1)) >> 8,
      a: (this.textCol.a * (a + 1)) >> 8
    };
  }
  ...
  async drawLineChart(x, y, w, h, data) {
    ...
    if ('enabled' in data) {
      ...
      for (let i = padX + 1; i < w; i++) {
        ...
        if (data.enabled[hour]) {
          await this.drawLine(x + i, y + h - padY - 1, x + i, y + h - padY - 11);
        }
      }
    }
