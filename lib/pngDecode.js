...
class PngDecode {
  ...
  reverseFilter(filtered) {
    ...
    const a = (x > 0) ? this.imageData[this.imageOffset - this.channels] : 0;
    const b = (y > 0) ? this.imageData[this.imageOffset - (this.width * this.channels)] : 0;
    const c = ((x > 0) && (y > 0)) ? this.imageData[this.imageOffset - (this.width * this.channels) - this.channels] : 0;
  ...
  async load(filename) {
    ...
    this.inflator.on('data', (data) => {
      for (let i = 0; i < data.length; i++) {
        ...
        if (isFilter) {
          ...
          this.imageData[this.imageOffset] = this.reverseFilter(data[i]);
          this.imageOffset++;
        }
        this.dataOffset += 1;
      }
    ...
    return fsp.readFile(filename)
      ...
      .then((readHandle) => {
        ...
        this.imageData = new Uint8Array(this.arrayData);
        this.imageOffset = 0;
        this.dataOffset = 0;
  ...
  async readHeader({ buffer, offset }) {
    return this.readChunk({ buffer, offset })
      .then((chunk) => {
        ...
        if (this.colourType !== COLOURTYPE.RGBA) return Promise.reject(new Error(`Bit Depth ${this.colourType} not supported yet`));
