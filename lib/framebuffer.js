...
findFile(CHAR_IMG)
// === SHARP ===
//   .then((file) => sharp(file)
//     .raw()
//     .toBuffer({ resolveWithObject: true })
//     .then(({ data, info }) => {
//       textImgReady = true;
//       textImg.data = data;
//       textImg.width = info.width;
//       textImg.height = info.height;
//       textImg.channels = info.channels;
//     }))
//   .catch((err) => { textImgError = err; });
// === PNGJS && JPEG-JS ===
  .then((file) => fs.createReadStream(file, { bufferSize: 1024 })
    .on('error', () => { textImgError = true; })
    .pipe(textImg)
    .on('parsed', () => { textImgReady = true; textImg.channels = 4; }));
...
class Framebuffer {
  ...
  pipe(target) {
    ...
    // === SHARP ===
    // return sharp(this.data, { raw: { width, height, channels } })
    //   .gif()
    //   .pipe(target);

    // === JPEG-JS ===
    // let jpegImageData = jpeg.encode({
    //   data: this.data,
    //   width,
    //   height
    // }, 10);
    ...
    // d.pipe(target);
    // d.end(jpegImageData.data);

    // === PNGJS ===
    // const tmp = new PNG({ width, height, colorType: 2, bgColor: { red: 80, green: 80, blue: 80 }});
    // for (let dd = 0; dd < width * height * 4; dd++) {
    //   tmp.data[dd] = this.data[dd];
    // }
    // return tmp.pack().pipe(target);

    // === MINIMALPNG ===
    if (this.encoder) delete this.encoder;
    this.encoder = new MinimalPng({ width, height, channels, imageData: this.data });
  ...
  async loadFile(filename) {
    return new Promise((resolve, reject) => {
      ...
      if (filename in cache) {
        ...
        fs.createReadStream(filename, { bufferSize: 1024 })
          .on('error', (err) => reject(err))
          .pipe(newImg)
          .on('parsed', () => { newImg.channels = 4; cache[filename] = newImg; resolve(newImg); });
        // === SHARP ===
        // sharp(filename)
        //   .raw()
        //   .toBuffer({ resolveWithObject: true })
        ...
        //     const newImage = { data, width, height, channels };
        //     this.cachedFiles[filename] = newImage;
        //     resolve(newImage);
        //   })
  ...
  // Replacement for pngjs bitblt (blits an image into another)
  ...
  async blitStretch(src, srcX, srcY, srcW, srcH, destX, destY, destW, destH) {
    for (let dy = 0; dy < destH; dy++) {
      for (let dx = 0; dx < destW; dx++) {
        ...
        const srcIdx = (src.width * (srcY + sy) + (srcX + sx)) * src.channels;

        const isBg = textImg.data[srcIdx] === 0
          && (!this.textUnder || dy !== (charHeight - 1));
  ...
  sample(src, startx, starty, w, h, x, y) {
    const srcIdx = (src.width * Math.round(starty + y) + Math.round(startx + x)) * src.channels;
    // const r = src.data[srcIdx + 0];
    // const g = src.data[srcIdx + 1];
    // const b = src.data[srcIdx + 2];
    const a = src.data[srcIdx] + 1;
    const isUnder = this.textUnder && y === (charHeight - 1);
    const isBg = (x < -0.5) || (y < -0.5) || (x > w - 0.5) || (y > h - 0.5)
     || (a === 1 && !isUnder);
    return isBg ? this.textBg
      : isUnder ? this.textCol
        : {
          r: (this.textCol.r * a) >> 8,
          g: (this.textCol.g * a) >> 8,
          b: (this.textCol.b * a) >> 8,
          a: (this.textCol.a * a) >> 8
        };
  }
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      switch (text[i]) {
        ...
        default:
          ...
          if (this.cursorX + this.getWidth(text.substring(i + 1).split(' ')[0] || ' ') >= this.cursorMaxX) {
            nextLine = true;
  ...
  async drawLineChart(x, y, w, h, data) {
    ...
    for (let i = 1; i < data.values.length; i++) {
      ...
      const y0 = ystart + (yspan * (maxVal - data.values[i - 1])) / yDelta;
      const y1 = ystart + (yspan * (maxVal - data.values[i])) / yDelta;

      // await this.drawLine(x0, y0, x1, y1);
      await this.drawLine(x0, y0, x0 + (x1 - x0) / 2, y0);
      await this.drawLine(x0 + (x1 - x0) / 2, y0, x0 + (x1 - x0) / 2, y1);
      await this.drawLine(x0 + (x1 - x0) / 2, y1, x1, y1);
    }
    // Draw enabled bar
    if ('group' in data) {
